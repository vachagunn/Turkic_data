...
def get_category(soup):
    try:
        category = soup.find('span', {'class': 'breadcrumb_last_link'})
...
def get_content(soup):
    try:
        article = soup.find('div', {'class': 'content-inner'})
...
def get_date(soup):
    try:
        date = soup.find('div', {'class': 'jeg_meta_date'}).get_text().strip()
        return date
    except:
        print('skipped date')
        return None
...
for page_number in range(1, 792):
    print(page_number)

    link = f"https://trt-tv.ru/tat/category/ya-alyklar/page/{page_number}/"
    page = get_page(link)
    if page is None:
        continue
    ...
    if news_block:
        news = news_block.find_all('article', {'class': 'jeg_post'})
    else:
        continue
    ...
    for link in links:
        ...
        if category or tags or text or date:
            with open(filename, mode='a', newline='', encoding='utf-8') as csv_file:
                writer = csv.writer(csv_file)
                writer.writerow([category, tags, text, date, link, 'https://trt-tv.ru/'])
