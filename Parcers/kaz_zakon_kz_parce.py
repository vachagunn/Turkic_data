...
output_file = open('kz_parced2.txt', 'a', encoding='utf-8')  # Открываем файл для записи (режим 'a' для добавления данных)
...
for main_url in main_urls:
    ...
    while True:  # Бесконечный цикл
        ...
        news_block = soup.find_all(class_='zmainCard_item')
        if len(news_block) == 0:
            break  # Если блок новостей пуст, прерываем цикл
        for post in news_block:
            if post.find("a"):
                hrefs.append('https://kaz.zakon.kz' + post.find("a").get("href"))
        ...
        if len(hrefs) != 0:
            ...
            for href in hrefs:
                ...
                main_content_element = soup_one_news.find(class_='content')
